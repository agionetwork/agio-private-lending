...
const t: Record<Lang, {
  ...
  en: {
    ...
    overview: "Overview",
    overviewDesc: "Agio exposes a 37-tool MCP server that any AI assistant or autonomous agent can connect to. Through MCP, external agents can interact with the full lending protocol: reading loan data, creating offers, accepting loans, repaying, and even swapping tokens via Jupiter. This is different from the built-in Auto Loan: while Auto Loan follows pre-configured rules, MCP-connected agents can reason about market conditions and make dynamic decisions.",
    ...
    availableToolsDesc: "The MCP server provides 37 tools organized into free (read-only) and paid (write) operations.",
  ...
  es: {
    ...
    overviewDesc: "Agio expone un servidor MCP con 37 herramientas al que cualquier asistente IA o agente autónomo puede conectarse. A través de MCP, agentes externos pueden interactuar con el protocolo completo de préstamos: leer datos, crear ofertas, aceptar préstamos, pagar e incluso intercambiar tokens vía Jupiter. Esto es diferente del Auto Loan integrado. El Auto Loan sigue reglas preconfiguradas, mientras que los agentes conectados por MCP pueden razonar sobre condiciones de mercado y tomar decisiones dinámicas.",
    ...
    availableToolsDesc: "El servidor MCP provee 37 herramientas organizadas en operaciones gratuitas (solo lectura) y de pago (escritura).",
  ...
  pt: {
    ...
    overviewDesc: "O Agio expõe um servidor MCP com 37 ferramentas ao qual qualquer assistente IA ou agente autônomo pode se conectar. Através do MCP, agentes externos podem interagir com o protocolo completo de empréstimos: ler dados, criar ofertas, aceitar empréstimos, pagar e até trocar tokens via Jupiter. Isso é diferente do Auto Loan integrado. O Auto Loan segue regras pré-configuradas, enquanto agentes conectados por MCP podem raciocinar sobre condições de mercado e tomar decisões dinâmicas.",
    ...
    availableToolsDesc: "O servidor MCP fornece 37 ferramentas organizadas em operações gratuitas (somente leitura) e pagas (escrita).",
  ...
  zh: {
    ...
    overview: "概述",
    overviewDesc: "Agio 公开了一个 37 工具的 MCP 服务器，任何 AI 助手或自主代理都可以连接。通过 MCP，外部代理可以与完整的借贷协议交互：读取贷款数据、创建报价、接受贷款、还款，甚至通过 Jupiter 兑换代币。这与内置的 Auto Loan 不同。Auto Loan 遵循预配置的规则，而通过 MCP 连接的代理可以推理市场条件并做出动态决策。",
    connecting: "连接您的代理",
